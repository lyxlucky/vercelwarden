...
export function FolderManagerDialog({ open, folders, onOpenChange, onCreate, onRename, onDelete }: {
  ...
  const submit = async () => { const nextName = name.trim(); if (!nextName) return; setBusy(true); try { if (editing) await onRename(editing.id, nextName); else await onCreate(nextName); setName(""); setEditing(null); } finally { setBusy(false); } };
  return (
    <>
      <Dialog open={open} onClose={() => handleOpenChange(false)}>
        <DialogTitle>管理文件夹</DialogTitle>
        <DialogContent>
          <DialogContentText sx={{ mb: 2 }}>删除文件夹只会解除项目归属，不会删除任何密码库项目。</DialogContentText>
          ...
          {folders.length === 0 ? <Typography color="text.secondary" sx={{ py: 3, textAlign: "center" }}>暂无文件夹</Typography> : (
            <List>{folders.map((folder) => <ListItem key={folder.id} divider secondaryAction={<Stack direction="row"><Tooltip title={`重命名 ${folder.name}`}><IconButton aria-label={`重命名 ${folder.name}`} onClick={() => { setEditing(folder); setName(folder.name); }}><EditOutlined /></IconButton></Tooltip><Tooltip title={`删除 ${folder.name}`}><IconButton aria-label={`删除 ${folder.name}`} color="error" onClick={() => setPendingDelete(folder)}><DeleteOutlineOutlined /></IconButton></Tooltip></Stack>}><ListItemText primary={folder.name} /></ListItem>)}</List>
          )}
        </DialogContent>
...
export function MoveItemsDialog({ open, count, folders, onOpenChange, onMove }: { open: boolean; count: number; folders: VaultFolderView[]; onOpenChange(open: boolean): void; onMove(folderId: string | null): Promise<void> }) {
  ...
  const [busy, setBusy] = useState(false);
  return <Dialog open={open} onClose={() => onOpenChange(false)}><DialogTitle>移动项目</DialogTitle><DialogContent><DialogContentText sx={{ mb: 2 }}>将 {count} 个项目移动到所选文件夹。</DialogContentText><FormControl><InputLabel id="move-folder-label">目标文件夹</InputLabel><Select labelId="move-folder-label" label="目标文件夹" value={folderId} onChange={(event) => setFolderId(event.target.value)}><MenuItem value="">无文件夹</MenuItem>{folders.map((folder) => <MenuItem key={folder.id} value={folder.id}>{folder.name}</MenuItem>)}</Select></FormControl></DialogContent><DialogActions><Button onClick={() => onOpenChange(false)}>取消</Button><Button variant="contained" disabled={busy} onClick={async () => { setBusy(true); try { await onMove(folderId || null); onOpenChange(false); } finally { setBusy(false); } }}>移动</Button></DialogActions></Dialog>;
}
...
export function ConflictDialog({ open, onOpenChange, onReload }: { open: boolean; onOpenChange(open: boolean): void; onReload(): Promise<void> }) {
  return <Dialog open={open} onClose={() => onOpenChange(false)}><DialogTitle>项目已在其他位置更改</DialogTitle><DialogContent><Alert severity="warning">为防止静默覆盖，本次操作没有修改冲突项目。</Alert><Typography sx={{ mt: 2 }}>刷新后请复核最新内容，再重新执行操作。</Typography></DialogContent><DialogActions><Button onClick={() => onOpenChange(false)}>关闭</Button><Button variant="contained" onClick={async () => { await onReload(); onOpenChange(false); }}>重新加载最新版本</Button></DialogActions></Dialog>;
}
